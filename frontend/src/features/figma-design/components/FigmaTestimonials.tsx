...
export function FigmaTestimonials() {
  ...
  const handlePrev = () => {
    setActiveIndex((p) => (p - 1 + FIGMA_TESTIMONIALS_DATA.length) % FIGMA_TESTIMONIALS_DATA.length);
  };

  const handleNext = () => {
    setActiveIndex((p) => (p + 1) % FIGMA_TESTIMONIALS_DATA.length);
  };
  ...
  return (
    <section className="relative mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 py-4 sm:py-6 select-none">
      ...
      {/* Modern Indicator Dots at Bottom */}
      <div className="mt-6 flex items-center justify-center">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900/90 rounded-full border border-zinc-800/80 shadow-md">
          {FIGMA_TESTIMONIALS_DATA.map((_, idx) => (
            <button
              key={idx}
              type="button"
              onClick={() => setActiveIndex(idx)}
